...
#define IR_SENSOR_PIN 23 //ir sensor
// Timer variables
...
#define WIFI_SSID "Jura"
#define WIFI_PASSWORD "jura2024"
...
void loop(){
  ...
  //
  if (temperature > 20 && air_quality < 120 && temperature < 30) {
    digitalWrite(LED_BLUE, HIGH);
    digitalWrite(LED_RED, LOW);
    digitalWrite(Buzzer, HIGH);
  } else if (temperature < 20 || air_quality > 120) {
    digitalWrite(LED_BLUE, LOW);
  ...
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 1000 || sendDataPrevMillis == 0)){
    ...
    Firebase.RTDB.setFloat(&fbdo, "DHT_22/Temperature", temperature);
    Firebase.RTDB.setFloat(&fbdo, "DHT_22/Humidity", humidity);
    Firebase.RTDB.setInt(&fbdo, "MQ_Sensor/AirQuality", air_quality);
    Firebase.RTDB.setInt(&fbdo, "Displacement/displacementX", displacementX);
    Firebase.RTDB.setInt(&fbdo, "Displacement/displacementY", displacementY);
    Firebase.RTDB.setInt(&fbdo, "Displacement/displacementZ", displacementZ);
    Firebase.RTDB.setInt(&fbdo, "IR_Sensor/State", irValue);

    // Check the buzzer control command
    String buzzerCommand;
    if (Firebase.RTDB.getString(&fbdo, "Control/BuzzerCommand", &buzzerCommand)) {
      if (buzzerCommand == "ON") {
        digitalWrite(Buzzer, LOW);
      } else if (buzzerCommand == "OFF") {
        digitalWrite(Buzzer, HIGH);
      }
    }

    // Check the LED1 control command
    String led1Command;
    if (Firebase.RTDB.getString(&fbdo, "Control/LED1Command", &led1Command)) {
      if (led1Command == "ON") {
        digitalWrite(LED_BLUE, HIGH);
      } else if (led1Command == "OFF") {
        digitalWrite(LED_BLUE, LOW);
      }
    }

    // Check the LED2 control command
    String led2Command;
    if (Firebase.RTDB.getString(&fbdo, "Control/LED2Command", &led2Command)) {
      if (led2Command == "ON") {
        digitalWrite(LED_RED, HIGH);
      } else if (led2Command == "OFF") {
        digitalWrite(LED_RED, LOW);
      }
    }
